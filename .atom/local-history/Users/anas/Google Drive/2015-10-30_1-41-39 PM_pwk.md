...
1. DNS Enumeration
	...
	- DNS Zone Transfers
		...
		# The result is a full dump of the zone file for the megacorpone.com domain, providing us a convenient list of IPs and DNS names for the megacorpone.com domain.
		```
		- Now Lets automate the process:
			...
			- To get the name servers for a given domain in a clean format, we can issue the following command.
				...
				# -d :: --delimiter=DELIM ;  -f ::  --fields=LIST select only these fields on each line;
				````
