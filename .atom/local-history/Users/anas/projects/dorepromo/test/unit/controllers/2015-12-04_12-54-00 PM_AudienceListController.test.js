...
describe('AudienceListController', function() {
  ...
  describe('#update()', function(){
    ...
    it('should update audience list name giving id', function(done){
      request(sails.hooks.http.app)
        .put('/audienclist/?id=Loremipsumdolorsitamet')
        .send({name: 'newName'})
        .end(function(err, res){
          assert.equal(res.status, 200, 'res.status equal 200');
          assert.equal(res.body.name, 'newName', 'res.body.name is equal to newName');
          done();
        });
    });

    it('should make sure that there is id provided', function(done){
      request(sails.hooks.http.app)
        .put('/audienclist')
        .send({name: 'newName'})
        .end(function(err, res){
