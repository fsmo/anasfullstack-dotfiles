...
apiApp.post('/test-goldfish', function(req, res) {
  ...
  var writeStream = fs.createWriteStream(pRequire('../record-analysis/' + chunkName + '.json'));

  req.pipe(writeStream);
  fs.readFile('sample.json', 'utf8', function(err, data) {
    if (err) throw err;
    wavData = JSON.stringify(data);
    var decodedStream = fs.createWriteStream(pRequire('../record-analysis/' + chunkName + '.wav'));
