...
apiApp.post('/test-goldfish',busboy(), function(req, res) {
  req.busboy.on('file', function(fieldname, file, filename, encoding, mimetype) {
    var ogg = wavToOgg(file);
    var baseUrl = 'http://localhost:' + global.port + '/api/2.0';

    var stream = ogg.process.stdout.pipe(
      request.post({
        url: baseUrl + '/analysis/new',
        headers: {
          accept: '*/*',
          cookie: req.headers.cookie
        },
        json: true
      }, function(error, response, song_body) {
        if(error) { return apiUtil.sendError(res, error); }

        if(response.statusCode !== 200) {
          res.status(response.statusCode).send(song_body);
          return;
        }

        request.post({
          url: baseUrl + '/song/' + song_body.song_id + '/activate',
          headers: {
            cookie: req.headers.cookie
          }
