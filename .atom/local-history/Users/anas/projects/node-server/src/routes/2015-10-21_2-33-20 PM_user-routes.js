...
function mergeJson() {
  var mergedName    = createRandomName();
  var mergedJsonDir = path.normalize(__dirname + '/../record-analysis/mergedJson_' + mergedName + '.json');
  var mergedWavDir  = path.normalize(__dirname + '/../record-analysis/mergedWav_' + mergedName + '.wav');
  var mergedJson    = fs.createWriteStream(mergedJsonDir);
  var mergedWav     = fs.createWriteStream(mergedWavDir);

  for (var i = 0; i < jsonChunksNames.length; i++) {
    var jsonFileDir = path.normalize(__dirname + '/../record-analysis/' + jsonChunksNames[i] + '.json');
    fs.readFile(jsonFileDir, 'utf8', function(err, data) {
      if (err) throw err;

      var wavData = JSON.stringify(data);

      var wavJsonData = wavData.substring(wavData.indexOf("base64,") + 7);
      str(wavJsonData).pipe(mergedJson,{ end: false });
    });
  }
  var mergeJsonStr = JSON.stringify(mergedJson);
  str(mergeJsonStr).pipe(base64.decode()).pipe(mergedWav);
}
...
apiApp.post('/test-goldfish', function(req, res) {
  ...
  var jsonFileDir = path.normalize(__dirname + '/../record-analysis/' + chunkName + '.wav');
  ...
  console.log('jsonChunksNames ' , jsonChunksNames);
  req.pipe(writeStream);
