...
apiApp.post('/test-goldfish', function(req, res) {
  ...
  var chunkName = makeid();
  var jsonFile = path.normalize('../record-analysis/' + chunkName + '.json');
  var writeStream = fs.createWriteStream(path.normalize('../record-analysis/' + chunkName + '.json'));

  req.pipe(writeStream);
  fs.readFile('sample.json', 'utf8', function(err, data) {
    if (err) throw err;
    wavData = JSON.stringify(data);
    var decodedStream = fs.createWriteStream(path.normalize('../record-analysis/' + chunkName + '.wav'));
