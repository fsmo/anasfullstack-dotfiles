...
apiApp.get('/current-user/location',auth.requireLogin, function(req, res) {
  res.json(getLocation(req));
});

apiApp.get('/current-user/delete-account',auth.requireLogin,
  function(req, res) {
    apiUtil.sendResponse(res, DeactivateManager.deleteAllUserInfo(req.user.user_id));
  }
);

apiApp.get('/current-user/user-song-counters',auth.requireLogin,
  function(req, res) {
    apiUtil.sendResponse(res, CountManager.getUserCounters(req.user.user_id));
  }
);
