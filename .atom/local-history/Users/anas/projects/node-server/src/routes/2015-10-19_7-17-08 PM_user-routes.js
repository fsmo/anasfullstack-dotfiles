...
apiApp.post('/test-goldfish', function(req, res) {
  var writeStream = fs.createWriteStream('sampleWav.wav');
  req.pipe(writeStream);
  fs.readFile('sampleWav.wav', 'utf8', function(err, data) {
    if (err) throw err;
    var wavData = JSON.stringify(data);
    var wavFile = wavData.substring(wavData.indexOf("base64,") + 6);
    console.log(wavFile.substring(0,10));
    // wavFile.pipe(base64.decode()).pipe(writeStream);
    // console.log(wavFile);
  });
