...
function createProfileIndexPages() {
  ...
  function writeXmlFile(userIds) {
    if (userIds.length > 0) {
      ...
      return P.each(userIds, function(userId) {
            if(!DeactivateManager.checkUserDeletedById(userId.user_id)){

            var xmlObj = {
                url       : 'http://my.scorecloud.com/user/' + userId.user_id,
                lastmod   : now.toISOString(),
                changefreq: 'weekly',
                priority  :0.75
            };
