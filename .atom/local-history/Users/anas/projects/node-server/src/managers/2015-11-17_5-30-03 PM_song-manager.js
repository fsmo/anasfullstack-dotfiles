...
function getSongsList(req, currentUser, localQuery, cached) {
  ...
  query.search = _.extend({
    ...
    return getCollectionsSongsView()
      ...
      .then(function(collectedSongs){
        console.log('collectedSongs.length', collectedSongs.length);

        return getRecordingSongViewModel()
        .find(query.search)
        .sort(query.sort + ' DESC')
        .then(function(userSongs){
          console.log('userSongs.length', userSongs.length);
          console.log('_.pluck(collectedSongs, "song_id"', _.pluck(collectedSongs, "song_id"));
          console.log('_.pluck(userSongs, "song_id"', _.pluck(userSongs, "song_id"));
          var collectedSongsIds = _.pluck(collectedSongs, "song_id");

          var diff = _.difference(_.pluck(collectedSongs, "song_id"), _.pluck(userSongs, "song_id"));
          console.log('diff', diff);
          var result = _.filter(userSongs, function(obj) { return diff.indexOf(obj.song_id) >= 0; });
          console.log(result);
          return result;

          // return P.map(userSongs, function(userSong){
          //     return  _.find(collectedSongs, function(collectedSong){
          //       return (collectedSong.song_id === userSong.song_id);
          //     });
