...
function getSongsBytag(req) {
  return getTagModel().findOne({name: req.query.tag})
    .then(function(tag) {
      if (tag) {

        return getTagstoObject()
          .find({tag_id: tag.tag_id})
      ...
      .find({
        ...
        .then(function(userSongs){
          return P.map(userSongs,function(song){
            ...
            var unCollectedSongs = _.filter(uncollected_songs,function(song){
              return song != null;
            });
