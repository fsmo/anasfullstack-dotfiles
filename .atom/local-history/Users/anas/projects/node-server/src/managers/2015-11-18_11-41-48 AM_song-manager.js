...
function getSongsList(req, currentUser, localQuery, cached) {
  if (req.query && req.query.tag){
    ...
    if (req.query.sort ==='popular'){ cached = true; }
    ...
    if (cached){
      ...
      console.log('Unsing unCached List');
...
function getALLBestSongsList(req) {
  ...
  return getSongsList(query, false, query, true);
}
...
function  getRecentBestSongsList(req) {
  ...
  return getSongsList(req, false, query, true);
}
