...
var now              = new Date();
var xmlFilesObjects  = [];
var linksLimit       = 20000;

var XMLWriter         = require('xml-writer');
var DeactivateManager = pRequire('/managers/deactivate-manager');
...
function createSongsIndexPages() {
  ...
  fileName        = 0,
  now             = new Date(),
  songFile        = createNewXmlFile(1, 'songs'),
  userFile        = createNewXmlFile(1,'users');
  ...
  function writeXmlFile(songs) {
    if (songs.length > 0) {
      ...
      if ((currentPage.limit * currentPage.page) % linksLimit === 0) {
        fileName += 1;
        songFile = createNewXmlFile(fileName, 'songs');
      }
      ...
      .then(function(songs) {
        return writeXmlFile(songs).then(function(nextPage) {
          ...
          if (nextPage) {
            ...
            log.debug('We finished all songs xml creation');
          }
        });
      });
  }
...
function createProfileIndexPages() {
  ...
  function writeXmlFile(users) {
    if (users.length > 0) {
      ...
      return P.each(users, function(user) {
        return getSongModel()
          .count({author_id: user.user_id, is_deleted: false, is_public: true})
          .then(function(userSongsCount){
            if(!DeactivateManager.checkUserDeleted(user)){
            var xmlObj = {
                url: 'http://my.scorecloud.com/user/' + user.user_id,
                lastmod: now.toISOString(),
                changefreq: 'weekly'
            };
            xmlObj.priority = userSongsCount > 0 ? 0.75 : 0.5;
            createXmlUrlElement(file, xmlObj);
          }
        });
      }).then(function() {
  ...
  function run() {
    return getUserModel()
      ...
      .paginate(currentPage)
      .then(function(users) {
        return writeXmlFile(users).then(function(nextPage) {
          currentPage.page += 1;
          if (nextPage) {
            return run();
          } else {
            log.debug('We finished all profiles xml creation');
          }
        });
      });
  }
...
function createindex() {
  ...
  return P.each(xmlFilesObjects, function(item) {
    ...
    xw.endElement();
    ws.end();
    log.debug('We finished all index xml creation');
  });
}
...
function uploadSiteMaps(){
  ...
  return P.each(xmlFiles,function(file){
    ...
    log.info('SiteMaps Upload Just Finished!');
  });
}
