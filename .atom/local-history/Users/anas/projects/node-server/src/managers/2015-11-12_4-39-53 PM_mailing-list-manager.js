...
function getNoNewSongsUsers() {
  return getSongModel()
    .find({is_deleted: false, creation_date: getPastDateTime(33)})
    .then(function(songs) {
      console.log('1', songs.length);
      return P.map(songs, function(song) {
        return getSongModel()
          ...
          .then(function(allUserSongs) {
            console.log('2', allUserSongs);
            if (allUserSongs === 0) {
              return song.author_id;
            }
          });
      })
      .then(function(noNewSongsUsersIds) {
        console.log('4', noNewSongsUsersIds);
        var filtereduserList =  _.filter(noNewSongsUsersIds, function(noNewSongsUser) {return !!noNewSongsUsersIds;});
        console.log('5', filtereduserList);
        return filtereduserList;
      });
    });
}
exports.getNoNewSongsUsers = getNoNewSongsUsers;
