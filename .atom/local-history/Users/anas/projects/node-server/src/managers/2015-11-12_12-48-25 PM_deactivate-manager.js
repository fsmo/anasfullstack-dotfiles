    ...
    .then(function(songs) {
      if (songs.length > 0) {
        ...
        P.each(songs, function(song) {
          return modelUtil.update(song, ['is_deleted'], {'is_deleted': true});
        });
      }
    });
}
...
function checkUserDeletedById(userId) {
  return getModel('user')
    .findOne({user_id: userId})
    .then(function(user) {
      return checkUserDeleted(user);
    });
}
exports.checkUserDeletedById=checkUserDeletedById;

function checkUserSubscribed(userId) {
  var now = new Date();
  return getModel('user_subscription_status')
    .findOne({user_id: userId, expiry_date: {'>': now}})
