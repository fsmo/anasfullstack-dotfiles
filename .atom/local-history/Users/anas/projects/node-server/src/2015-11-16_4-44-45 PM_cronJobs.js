...
(function initCron(handler) {
  ...
  loadCredentials(function(err, credentials) {
    ...
    orm.init(function() {
      ...
      //refreshmaterialized_song_view cron job runs daily at 04:30
      var updateSongMatView = createCronJob('00 15 10 * * *',function() {
        log.info('uploadSiteMaps cron job started at 04:30 is now runnning!!');
        orm.query('REFRESH MATERIALIZED VIEW materialized_song_view;')
      });
