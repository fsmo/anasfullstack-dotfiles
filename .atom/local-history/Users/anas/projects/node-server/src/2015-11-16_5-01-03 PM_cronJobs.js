...
(function initCron(handler) {
  ...
  loadCredentials(function(err, credentials) {
    ...
    orm.init(function() {
      ...
      var updateSongMatView = createCronJob('00 02 17 * * *',function() {
        log.info('updateSongMatView cron job started at 04:30 is now runnning!!');
        orm.getModel('materialized_song_view').query('REFRESH MATERIALIZED VIEW materialized_song_view;',function(result){
          console.log(result);
