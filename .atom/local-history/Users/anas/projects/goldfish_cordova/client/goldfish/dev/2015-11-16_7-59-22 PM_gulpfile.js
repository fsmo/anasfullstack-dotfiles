...
gulp.task('injectInHtml', function() {
  ...
  var config     = gulp.src('../www/js/config.js');
  var index    = gulp.src('../www/js/index.js');
  var jsSources  = gulp.src(['../www/js/**/**/**/*', '!../www/js/jquery.js', '!../www/js/angular.js', '!../www/js/config.js', '!../www/js/index.js']);
  var cssSources = gulp.src(['../www/css/**/**/**/*']);
  var target     = gulp.src('index.html');

  target.pipe(plugins.inject(series(index, jquery, angular, config, jsSources, cssSources), {relative: true,ignorePath: "../www/"}))
    .pipe(gulp.dest('./../www'))
...
gulp.task('default', function() {
  gulp.run('lr-server', 'scripts', 'less', 'css', 'views', 'images', 'fonts', 'injectInHtml');

  gulp.watch('features/**/**/*.js', function(event) {
    gulp.run('scripts');
  });
