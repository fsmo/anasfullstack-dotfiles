App.service('webRecorderService', [function(){
  ...
  function encodeBuffersToWav(input) {
    ...
    console.log('Encoding output: ' , output);
    return output;
  }
  ...
  this.startStreaming = function() {
    ...
    setInterval(function() {
      ...
      if (isFirstChunk) {
        ...
        self.audioStreamer.exportWAV(function(blob) {
          self.audioStreamer.clear();
          if (blob.size > 44) {
            var reader = new FileReader();
            reader.onload = function(event) {
              var wavfile = {};
              wavfile.fname = chunkNumber + ".wav";
              wavfile.data = event.target.result;
              Meteor.subscribe("stream", wavfile);
            };
        ...
        var audioWavBuffer = self.audioStreamer.getBuffers(function(buffers){
          console.log('buffers', buffers);
          var encodedWav = encodeBuffersToWav(buffers);
          console.log('encodedWav' , encodedWav);
          console.log('encodedWav ' , encodedWav);
          var reader = new FileReader();
          reader.onload = function(event) {
            var wavfile = {};
          ...
          var wavBlob = new Blob([audioWavBuffer], { type: 'audio/wav' });
