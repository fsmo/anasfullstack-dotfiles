App.service('webRecorderService',[function(){
  ...
  this.getStreamBuffers = function(){
    ...
    newSource.start(0);
      ...
      console.log(buffers);
      Meteor.subscribe("getAudioStream", newSource);
    });
  };
  ...
  this.initAudio =  function () {
    ...
    navigator.getUserMedia({
      ...
      function (stream) {
        ...
        var backgroundMusic = audioContext.createMediaElementSource(document.getElementById("back"));
        var analyser = audioContext.createAnalyser();
        var mixedOutput = audioContext.createMediaStreamDestination();
        microphone.connect(analyser);
        analyser.connect(mixedOutput);
        backgroundMusic.connect(mixedOutput);
        navigator.requestAnimationFrame(drawAnimation);
