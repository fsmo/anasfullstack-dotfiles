App.service('webRecorderService', [function(){
  ...
  function sendBlob(blob, callback) {
    ...
    reader.onload = function(event) {
      Meteor.call('stream', new ArrayBuffer(event.target.result), function onDone() {
        if(callback) { callback(); }
      });
      // Meteor.call('stream', new Uint8Array(event.target.result), function onDone() {
      //   if(callback) { callback(); }
      // });
    };
  ...
  function uploadAudioStreamChunk(callback) {
  ...
  this.stopStreaming = function(){
    ...
    uploadAudioStreamChunk(function() {
