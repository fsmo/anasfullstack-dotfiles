...
/*
currentStreamChunks: {  // crrStrmChnks
  someSessionId: {
    lastChunk: 3
...
Meteor.methods({
  ...
  stream: function(data, session, chnkNum) {
    if (chnkNum - crrStrmChnks[session] === 1){
      ...
      console.log('stream', session,'data length: ', data.length, 'chnkNum', chnkNum);
      ...
      console.warn('stream', session,'data length: ', data.length, 'wrong chnkNum = ', chunkNumber);
