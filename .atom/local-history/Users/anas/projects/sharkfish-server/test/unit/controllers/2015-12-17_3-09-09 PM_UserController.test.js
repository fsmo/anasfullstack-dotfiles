var request = require('supertest');
var assert = require('chai').assert;

describe('#UserController', function() {
  'use strict';

  var agent = request.agent('http://localhost:1337') ;

  describe('create', function() {

    it('should create new user given email and a password', function(done) {
      request(sails.hooks.http.app)
        .post('/user')
        .send({
          email: 'anas@gmail.com',
          password: '123456',
          _id: 'Loremipsumdolorsita'
        })
        .end(function(err, res) {
          assert.isNull(err, 'err equal to null');
          assert.equal(res.status, 201, 'res.status equal to 201');
          assert.equal(res.body.email, 'anas@gmail.com', 'res.body.email equal to anas@gmail.com');
          assert.isNotNull(res.body.id, 'res.body.id is not null');
          assert.isUndefined(res.body.password, 'res.body.password equal to null');
          done();
        });
    });

    it('should create new user given email (case insensetive) and a password', function(done) {
      request(sails.hooks.http.app)
        .post('/user')
        .send({
          email: 'Anas@Anas.com',
          password: '123456'
        })
        .end(function(err, res) {
          assert.isNull(err, 'err equal to null');
          assert.equal(res.status, 201, 'res.status equal to 201');
          assert.equal(res.body.email, 'anas@anas.com', 'res.body.email equal to anas@anas.com');
          assert.isNotNull(res.body.id, 'res.body.id is not null');
          assert.isUndefined(res.body.password, 'res.body.password equal to null');
          done();
        });
    });

    it('should create new user given email, password & id', function(done) {
      request(sails.hooks.http.app)
        .post('/user')
        .send({
          email: 'anas.ieee@gmail.com',
          password: '123456',
          _id: 'Loremipsumdolorsitamet'
        })
        .end(function(err, res) {
          sails.log.debug('res', res.body.email);
          assert.isNull(err, 'err equal to null');
          assert.equal(res.status, 201, 'res.status equal to 201');
          assert.equal(res.body.email, 'anas.ieee@gmail.com', 'res.body.email equal to anas.ieee@gmail.com');
          assert.equal(res.body.id, 'Loremipsumdolorsitamet', 'res.body.id equal to Loremipsumdolorsitamet');
          assert.isUndefined(res.body.password, 'res.body.password equal to null');
          done();
        });
    });

    it('should make sure unique email added', function(done) {
      request(sails.hooks.http.app)
        .post('/user')
        .send({
          email: 'anas.ieee@gmail.com',
          password: '123456'
        })
        .end(function(err, res) {
          assert.isNull(err, 'err equal to null');
          assert.equal(res.status, 403, 'res.status equal to 403');
          done();
        });
    });

    it('should make sure a real email added', function(done) {
      request(sails.hooks.http.app)
        .post('/user')
        .send({
          email: 'anasl.com',
          password: '123456'
        })
        .end(function(err, res) {
          assert.isNull(err, 'err equal to null');
          assert.equal(res.status, 403, 'res.status equal to 403');
          done();
        });
    });

    it('should make sure a password is added', function(done) {
      request(sails.hooks.http.app)
        .post('/user')
        .send({
          email: 'anas@gmail.com'
        })
        .end(function(err, res) {
          assert.isNull(err, 'err equal to null');
          assert.equal(res.status, 400, 'res.status equal to 400');
          done();
        });
    });

    it('should make sure a password length of 6', function(done) {
      request(sails.hooks.http.app)
        .post('/user')
        .send({
          email: 'anas@gmail.com',
          password: '126'
        })
        .end(function(err, res) {
          assert.isNull(err, 'err equal to null');
          assert.equal(res.status, 403, 'res.status equal to 403');
          done();
        });
    });
  });

  describe('#read()', function() {
    before(function(done) {
        agent
          .post('/login/email')
          .send({
            email: 'anas@gmail.com',
            password: '123456'
          })
          .end(function(err, res) {
            if (err) { return done(err);}
            assert.equal(res.status, 200, 'res.status equal to 200');
            done();
          });
      });

    it('should return one user object by id', function(done) {
      agent
        .get('/user/?id=Loremipsumdolorsitamet')
        .end(function(err, res) {
          assert.equal(res.status, 200, 'res.status equal 200');
          assert.isObject(res.body, 'body is object');
          assert.equal(res.body.email, 'anas.ieee@gmail.com', 'res.body.email, should equal to anas.ieee@gmail.com');
          assert.isUndefined(res.body.password, 'res.body.password should be undefined');
          done();
        });
    });

    it('should not return  user because agent is logged in', function(done) {
      request(sails.hooks.http.app)
        .get('/user/?id=Loremipsumdolorsitamet')
        .end(function(err, res) {
          assert.equal(res.status, 403, 'res.status equal 403');
          done();
        });
    });

    it('should return error not found for searching not found List', function(done) {
      agent
        .get('/user/?id=Loremipsumdolorsitamet2324')
        .end(function(err, res) {
          assert.equal(res.status, 404, 'res.status equal 404');
          done();
        });
    });

    it('should return an array containing all users that exist', function(done) {
      agent
        .get('/user')
        .end(function(err, res) {
          assert.equal(res.status, 200, 'res.status equal 200');
          assert.isArray(res.body, 'res.body is Array');
          assert.equal(res.body.length, 3, 'res.body.length is equal to 3');
          done();
        });
    });
  });

  describe('#update()', function() {

    it('should not update user because agent is not the owner of the account', function(done) {
      agent
        .put('/user/?id=Loremipsumdolorsitamet')
        .send({
          name: 'newName',
          location: 'USA'
        })
        .end(function(err, res) {
          assert.equal(res.status, 403, 'res.status equal 403');
          done();
        });
    });

    it('should update user name giving id', function(done) {
      agent
        .put('/user/?id=Loremipsumdolorsita')
        .send({
          name: 'newName',
          location: 'USA'
        })
        .end(function(err, res) {
          assert.equal(res.status, 200, 'res.status equal 200');
          assert.equal(res.body[0].id, 'Loremipsumdolorsita', 'res.body[0].id is equal to Loremipsumdolorsita');
          assert.equal(res.body[0].name, 'newName', 'res.body[0].name is equal to newName');
          assert.equal(res.body[0].location, 'USA', 'res.body[0].target is equal to USA audience');
          done();
        });
    });

    it('should make sure that there is an id provided', function(done) {
      agent
        .put('/user')
        .send({
          name: 'newName'
        })
        .end(function(err, res) {
          assert.equal(res.status, 400, 'res.status equal 400');
          done();
        });
    });
  });

  describe('#delete', function() {

    it('should delete user given id', function(done) {
      agent
        .delete('/user/?id=Loremipsumdolorsita')
        .end(function(err, res) {
          assert.isNull(err, 'error is equal to null');
          assert.equal(res.status, 200, 'res.status is equal to 200');
          assert.equal(res.text, 'User Deleted Successfully!!', 'res.text is equal to User Deleted Successfully!!');
          done();
        });
    });

    it('should make sure that id is provided', function(done) {
      agent
        .delete('/user')
        .end(function(err, res) {
          assert.isNull(err, 'error is equal to null');
          assert.equal(res.status, 400, 'res.status is equal to 400');
          done();
        });
    });

    it('should make sure that the user is there first', function(done) {
      agent
        .delete('/user/?id=Loremipsumdolorsitamet123')
        .end(function(err, res) {
          assert.isNull(err, 'error is equal to null');
          assert.equal(res.status, 404, 'res.status is equal to 404');
          done();
        });
    });
  });

  describe('#forgotPassword', function() {

    it('Should send an email with a token url and insert the token in db', function(done) {
      request(sails.hooks.http.app)
        .post('/forget-my-password')
        .send({
          email: 'anas.ieee@gmail.com'
        })
        .end(function(err, res) {
          assert.isNull(err, 'error is equal to null');
          assert.equal(res.status, 200, 'res.status is equal to 200');
          done();
        });
    });

    it('Should not send any thing if the account is deleted', function(done) {
      request(sails.hooks.http.app)
        .post('/forget-my-password')
        .send({
          email: 'anas@gmail.com'
        })
        .end(function(err, res) {
          assert.isNull(err, 'error is equal to null');
          assert.equal(res.status, 404, 'res.status is equal to 404');
          done();
        });
    });

    it('Should not send any thing if the account is not found', function(done) {
      request(sails.hooks.http.app)
        .post('/forget-my-password')
        .send({
          email: 'blabla@gmail.com'
        })
        .end(function(err, res) {
          assert.isNull(err, 'error is equal to null');
          assert.equal(res.status, 404, 'res.status is equal to 404');
          done();
        });
    });

  });

  describe('updatePassword', function() {

    it('Should Create a new user with a known password and resetPasswordToken', function(done) {
      request(sails.hooks.http.app)
        .post('/user')
        .send({
          _id: 'Loremipsumdolorsitametbla',
          email: 'anas.aboureada@doremir.com',
          password: 123456,
          resetPasswordToken: '5f09dd037a4284aacda48fea802d30b0e78e5299',
          resetPasswordExpires: new Date(Date.now() + 60 * 60 * 1000)
        })
        .end(function(err, res) {
          assert.isNull(err, 'err is equal to null');
          assert.equal(res.status, 201, 'res.status is equal to 201');
          assert.equal(res.body.id, 'Loremipsumdolorsitametbla', 'res.body.id is equal to Loremipsumdolorsitametbla');
          done();
        });
    });

    it('Should Create a new user with a know npassword and resetPasswordToken', function(done) {
      request(sails.hooks.http.app)
        .post('/user')
        .send({
          _id: 'Loremipsumdolorsitametblabla',
          email: 'anas@doremir.com',
          password: 123456,
          resetPasswordToken: '5f09dd037a4284aacda48fea802d30b',
          resetPasswordExpires: new Date(Date.now() - 60 * 60 * 1000)
        })
        .end(function(err, res) {
          assert.isNull(err, 'err is equal to null');
          assert.equal(res.status, 201, 'res.status is equal to 201');
          assert.equal(res.body.id, 'Loremipsumdolorsitametblabla', 'res.body.id equal Loremipsumdolorsitametblabla');
          done();
        });
    });

    it('Should update user with a new password given newPassword and a valid token', function(done) {
      request(sails.hooks.http.app)
        .post('/update-my-password')
        .send({
          newPassword: 123456789,
          token: '5f09dd037a4284aacda48fea802d30b0e78e5299'
        })
        .end(function(err, res) {
          assert.isNull(err, 'err is equal to null');
          assert.equal(res.status, 200, 'res.status is equal to 200');
          done();
        });
    });

    it('Should make sure of a valid resetPasswordExpires', function(done) {
      request(sails.hooks.http.app)
        .post('/update-my-password')
        .send({
          newPassword: 123456789,
          token: '5f09dd037a4284aacda48fea802d30b'
        })
        .end(function(err, res) {
          assert.isNull(err, 'err is equal to null');
          assert.equal(res.status, 403, 'res.status is equal to 403');
          done();
        });
    });

    it('Should make sure of a valid token', function(done) {
      request(sails.hooks.http.app)
        .post('/update-my-password')
        .send({
          newPassword: 123456789,
          token: '5f09dd037a4284aacda48fe30b'
        })
        .end(function(err, res) {
          assert.isNull(err, 'err is equal to null');
          assert.equal(res.status, 403, 'res.status is equal to 403');
          done();
        });
    });
  });
});
