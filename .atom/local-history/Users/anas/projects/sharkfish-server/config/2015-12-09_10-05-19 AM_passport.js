...
if (process.env.GOOGLE_AUTH) {
  twitterAuth = JSON.parse(process.env.GOOGLE_AUTH);
}else {
  twitterAuth = require('./local.js').socialIds.google;
}

if (process.env.TWITTER_AUTH) {
  googleAuth = JSON.parse(process.env.TWITTER_AUTH);
}else {
  googleAuth = require('./local.js').socialIds.twitter;
}
